...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.,channels_first=False):
        ...
        # print(f"Inside each MLP Block: Linear({in_features}, {hidden_features}), Act Layer, Linear({hidden_features}, {out_features})")
    def forward(self, x):
        x = self.fc1(x)
        x = self.act(x)
        x = self.drop(x)
...
class SS2D_SSM(nn.Module):
    def __init__(self,
        ...
        for _ in range(self.n_layers):
            encoder_layer = SS2D(d_model=self.d_model, d_state=self.d_state,forward_type="v3noz")
            mapping_layers.append(encoder_layer)
            self.norms.append(nn.LayerNorm(d_model))

        self.patch_size = patch_size
        self.input_and_post_norm = _make_patch_embed(in_chans=d_input, embed_dim=d_model, patch_size=patch_size, patch_norm=True, norm_layer=nn.LayerNorm,channel_first=False)
        self.mapping_layers = nn.Sequential(*mapping_layers)
        #self.output = nn.Linear(d_model, d_output)
        self.classifier = nn.Sequential(OrderedDict(
            norm=nn.LayerNorm(d_model),  # B,H,W,C
            permute=(Permute(0, 3, 1, 2) if not self.channel_first else nn.Identity()),
            avgpool=nn.AdaptiveAvgPool2d(1),
            flatten=nn.Flatten(1),
            head=nn.Linear(d_model, d_output),
        ))

    def forward(self, x, state=None):
        print(f"x before input_and_post_norm: x.shape={x.shape}")
        x = self.input_and_post_norm(x)
        num_layer = 0
        ...
        for layer in self.mapping_layers:
            ...
            num_layer += 1
        print(f"x before classifier: x.shape={x.shape}")
        #output = self.output(x.mean(dim=1))
        output = self.classifier(x)
        print(f"output after classifier: output.shape={output.shape}")
        return output
...
class SS2D(nn.Module, mamba_init):
    def __init__(
        ...
        **kwargs,
        ...
        # print(f"Inside each ss2d block {kwargs}")
    ...
    def __initv2__(
        ...
        **kwargs,    
        ...
        if initialize in ["v0"]:
            ...
            # print(
            #     f"Inside initv2 block of SS2D: d_inner {d_inner}, dt_rank {dt_rank}, in_proj: Linear({d_model}, {d_proj}), "
            #     f"x_proj_w {self.x_proj_weight.shape}, out_proj: Linear({d_inner}, {d_model}),"
            #     f"A_logs: (4*d_inner,N): {self.A_logs.shape},"
            #     f"Ds: (4*d_inner): {self.Ds.shape}")
...
class VSSBlock(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        # print(f"Inside each VSSB Block,"
        #       f" MLP Branch {self.mlp_branch} SSM Branch {self.ssm_branch}, D_iiner/Hidden Dim {hidden_dim}, d_state {ssm_d_state}")
        if self.ssm_branch:
...
class VSSM(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        # config_path = "/home/gamir/edocohen/talr/DeepLearningCourseProject/vits-robustness-torch/configs/xcit-adv-training-cifar10.yaml"
        # with open(config_path, "r") as file:
        #     yaml_cfg = yaml.safe_load(file)
        # self.default_cfg = yaml_cfg
        if num_classes == 10:
            self.default_cfg = {
                ...
                'interpolation': 'bicubic',  # Interpolation method for resizing. This is the default in 'timm.models.resolve_pretrained_cfg'
                'crop_pct': 0.875,  # Crop percentage (for augmentations, if needed)
                'input_range': (0.0, 1.0),  # Normalized range for input
            }
        elif num_classes == 1000: #Imagenet. imm.models.resolve_pretrained_cfg takes as default: IMAGENET_DEFAULT_MEAN,IMAGENET_DEFAULT_STD, input_size=(3,224,224), DEFAULT_CROP_PCT=0.875
            print(f"creating  Imagenet default_cfg")
            self.default_cfg = {
                'num_classes': 1000,
                'interpolation': 'bicubic', # Interpolation method for resizing. This is the default in 'timm.models.resolve_pretrained_cfg'
            }
        else:
            print(f"num_classes {num_classes} is not supported")
            exit(1)
    ...
    def reset_classifier(self, num_classes, global_pool=''):
        self.num_classes = num_classes
        self.classifier.head = nn.Linear(self.num_features, num_classes) if num_classes > 0 else nn.Identity()
    ...
    def _make_layer(
        ...
        **kwargs,
        ...
        depth = len(drop_path)
        blocks = []
        # print(f"Number of VSSM BLock in this layer {depth}, Each Block has "
        #       f"Hidden Dim {dim}, SSM_D_STATE {ssm_d_state}, SSM_CONV_BIAS {ssm_conv_bias}, "
        #       f"SSM Ratio {ssm_ratio} SSM DT RANK {ssm_dt_rank} MLP Ratio {mlp_ratio}, forward type {forward_type}")
    ...
    def forward(self, x: torch.Tensor):
        ...
        for layer in self.layers:
            x = layer(x)
            # print(f"Shape after layer {layer_id} : {x.shape}")
            # layer_id += 1
        x = self.classifier(x)
        return x
...
# def _create_vssm_tiny_v2(variant, pretrained=False, default_cfg=None, **kwargs):
#     config_path = "/home/gamir/edocohen/talr/DeepLearningCourseProject/vits-robustness-torch/configs/xcit-adv-training-cifar10.yaml"
#     with open(config_path, "r") as file:
#         default_cfg = yaml.safe_load(file)
#     model = build_model_with_cfg(VSSM, variant, pretrained=pretrained, default_cfg=default_cfg, **kwargs)
#     return model
#
#
# @register_model
# def vssm_tiny_v2(pretrained=False, **kwargs):
#     model_args = dict(patch_size=4,
#                 in_chans=3,
#                 num_classes=1000,
#                 depths=[2, 2, 5, 2],
#                 dims=96,
...
#                 use_checkpoint=False,)
#     return _create_vssm_tiny_v2('vssm_tiny_v2', pretrained=pretrained, default_cfg=None, **model_args)

@register_model
def vssm_tiny_v2(pretrained=False, **kwargs): # kwargs['num_classes']=10
    model = VSSM(
            patch_size=4,
            in_chans=3,
...
if __name__ == "__main__":
    ...
    for h_dim in hidden_dims:
        block_1 = VSSBlock(hidden_dim=h_dim,
                           ssm_d_state=16,
                           ssm_conv_bias=True,
                           mlp_ratio=0.0)
        # v0 block with mlp branch
        block_2 = VSSBlock(hidden_dim=h_dim,
                           ssm_d_state=16,
                           ssm_conv_bias=True,
                           mlp_ratio=4.0)
        # v2 block
        block_3 = VSSBlock(hidden_dim=h_dim,
                           ssm_d_state=1,
                           ssm_conv_bias=False,
                           forward_type='v3noz',
                           mlp_ratio=4.0)

        # compute total parameters in block_1 and 2
        print(f"parameters for dimension {h_dim}")
        params_block_1 = sum(p.numel() for p in block_1.parameters() if p.requires_grad)
        params_block_2 = sum(p.numel() for p in block_2.parameters() if p.requires_grad)
        params_block_3 = sum(p.numel() for p in block_3.parameters() if p.requires_grad)

        print(params_block_1/1e6)
        print(params_block_2/1e6)
        print(params_block_3/1e6)