...
class PerformerFeatures(FeatureMap):
    ...
    def __init__(
        ...
        self.register_buffer(
            "projection_matrix", torch.zeros(self.query_dims, self.n_features)
        )
        self._calls = -1
    ...
    def forward_queries(self, x):
        return softmax_kernel(
            x, projection_matrix=self.projection_matrix, is_query=True
        )

    def forward_keys(self, x):
        return softmax_kernel(
            x, projection_matrix=self.projection_matrix, is_query=False
        )
