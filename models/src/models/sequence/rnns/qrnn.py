...
from src.models.hippo.transition import (
    ...
    LagTCumsumAdaptiveTransition,
    TLagTCumsumAdaptiveTransition,
)
...
class VariableMemoryProjection(nn.Module):
    ...
    def forward(self, inputs, dt=None):
        ...
        As = self.transition.bilinear(
            dt, I, 0
        )  # (N, L, B, M, N) # NOTE due to the broadcasting here, the ManualTransition actually swaps axes back for efficiency; can potential save if this axis reordering is too slow [probably not a bottleneck]
        As = As.permute((1, 2, 3, 0, 4))  # (L, B, M, N, N)
        # TODO this A might be transposed; should print to compare
        # print(As.shape)
        Bs = self.transition.bilinear(
            dt, inputs.new_zeros(self.order), 1
        )  # (L, B, M, N)
...
class HiPPOQRNN(SequenceModule):
    ...
    def __init__(
        ...
        preact_args = [
            self.d_input + self.memory_size * self.memory_order,
            self.d_model,
            True,
        ]
        ...
        if self.variable:
            ...
            if measure in ["lagt", "tlagt"]:
                self.memory_proj = ToeplitzMemoryProjection(
                    memory_order, measure, measure_args
                )
            else:
    ...
    def forward(self, inputs, return_output=False):
        ...
        mx = torch.cat(
            (m.view(L, B, self.memory_size * self.memory_order), inputs), dim=-1
        )  # length, batch, d_input
