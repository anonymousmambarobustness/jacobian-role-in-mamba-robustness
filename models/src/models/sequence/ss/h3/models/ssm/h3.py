...
try:
    from ...ops.fftconv import fftconv_func
except ImportError:
    fftconv_func = None
...
class H3(nn.Module):
    ...
    def __init__(
        ...
        #self.mlp = nn.Sequential(nn.GELU(),nn.Dropout(dropout),nn.Linear(self.d_model, 2*self.d_model),nn.GELU(),nn.Linear(2*self.d_model, self.d_model))
        #self.drop2 = nn.Dropout(dropout)
        #self.norm2 = nn.LayerNorm(d_model)
        self.output_linear = nn.Linear(self.d_model, self.d_model)

    def forward(self, u, state=None, inference_params=None):
        """
        u: (B L H)
