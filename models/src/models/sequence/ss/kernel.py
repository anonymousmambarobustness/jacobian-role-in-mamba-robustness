...
import os
os.environ['CUDA_PATH'] = '/opt/conda/envs/gjqpy39/include'

import models.src.models.hippo.hippo as hippo
from models.src.models.functional.krylov import krylov, power
...
try:
    ...
    print("CUDA extension for cauchy multiplication found.")
except:
    print(
...
try:
    ...
    has_pykeops = False
    from models.src.models.functional.cauchy import cauchy_slow
...
def generate_kernel(H, N, L, measure="legs", rank=1):
    ...
    C = contract("ij, ... j -> ... i", V.conj().transpose(-1, -2), _C.to(V))  # V^* B
    A = torch.diag_embed(_conj(w)) - contract(
        "... r p, ... r q -> ... p q", _conj(p), _conj(p).conj()
    )
