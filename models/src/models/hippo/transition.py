...
from src.models.functional.toeplitz import (
    causal_convolution,
    causal_convolution_inverse,
    causal_convolution_inverse_wrong,
    construct_toeplitz,
)

# TODO figure out if we actually need this
try:
    from extensions.legt.legt import (
        legt_gbt_forward,
        legt_gbt_backward,
        legt_gbt_forward_t,
        legt_gbt_backward_t,
...
class AdaptiveTransition(nn.Module):
    def __init__(self, N, params, trainable=False, lr=1.0, batch=()):
        ...
        if self.trainable:
            ...
            assert (
                batch == ()
            ), "If not learnable, Transition should not have a batch dimension"
            for name, p in params.items():
    ...
    def gbt_B(self, dt, alpha=0.5):
        B = self.bilinear(
            dt, dt.new_zeros(*dt.shape, self.N), dt.new_ones(1), alpha=alpha
        )  # (..., N)
        return B
...
class CumsumAdaptiveTransition(AdaptiveTransition):
    ...
    def precompute_backward(self, delta):  # TODO should be called inverse?
        ...
        if self.N == 1:
            return 1.0 / (1.0 + self.a * delta + delta)
        ...
        pows = (
            b**self.arange
        )  ## TODO benchmark against cumprod or cumsum in log space
        tail = s * pows
    ...
    def precompute_gbt_A(self, delta, alpha=0.5):
        ...
        c = self.precompute_forward((1.0 - alpha) * delta)
...
class TLagTCumsumAdaptiveTransition(CumsumAdaptiveTransition):
    ...
    def __init__(self, N, beta=1.0):
        super().__init__(N, -(1.0 - beta) / 2, beta)
...
class LegTAdaptiveTransition(AdaptiveTransition):
    ...
    def forward_mult(self, u, delta, transpose=False):
        if transpose:
            return legt_gbt_forward_t(
                delta, u, transpose=True
...
def _diag(N, c):
    return F.pad(torch.ones(N - 1), (1, 1)) * c
...
class LegTTriDInverseAdaptiveTransition(TriDInverseAdaptiveTransition):
    def __init__(self, N, corners=3, **kwargs):
        ...
        # p = torch.ones(N)
        dl = _diag(N, -0.5)  # + F.pad(torch.randn(N-1)*1e-4, (1, 1))
        du = _diag(N, 0.5)  # + F.pad(torch.randn(N-1)*1e-4, (1, 1))
        ...
        if corners == 0:
            ...
            raise NotImplementedError
        c = torch.ones(N) * 0.0  # + torch.randn(N)*1e-4
...
class GLagTToeplitzAdaptiveTransition(ToeplitzAdaptiveTransition):
    def __init__(self, N, alpha=0.0, beta=0.01, **kwargs):
        ...
        # b = torch.ones(N)
        b = (
            ss.binom(alpha + np.arange(N), np.arange(N))
