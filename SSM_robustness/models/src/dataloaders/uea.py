...
def preprocess_data(
    ...
    train_X, val_X, test_X = split_data(
        X, y
    )  # TODO split data should just return y? or list of indices corresponding to splits
...
def process_data(dataset_name, intensity):
    ...
    X = torch.stack(
        [
            torch.stack([_pad(channel, maxlen) for channel in batch], dim=0)
            for batch in X
        ],
        dim=0,
    )
...
def get_data(
    ...
    try:
        ...
        (
            ...
            num_classes,
            input_channels,
        ) = process_data(dataset_name, intensity)
        save_data(
            loc,
            train_X=train_X,
            val_X=val_X,
            ...
            num_classes=torch.as_tensor(num_classes),
            input_channels=torch.as_tensor(input_channels),
        )
...
def _subsample(X, hz=1, uniform=True, mask=False):
    ...
    if uniform:
        ...
        generator = torch.Generator().manual_seed(56789)
        time_gen = (
            lambda: torch.randperm(L, generator=generator)[: int(L * hz)].sort().values
        )
