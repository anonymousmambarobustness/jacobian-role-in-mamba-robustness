    ...
    def __init__(
        ...
        **kwargs
        ...
        if self.gate is not None:
            ...
            if self.architecture["hh"]:
                print(
                    "input to hidden size, memory to hidden size, hidden size:",
                    self.input_to_d_model,
                    self.memory_to_d_model,
                    self.d_model,
                )
                preact_args[0] += self.d_model
            self.W_gxm = Gate(
                self.d_model, preact_ctor, preact_args, mechanism=self.gate
            )
