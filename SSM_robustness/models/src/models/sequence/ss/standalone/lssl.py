""" Standalone file implementing the (Structured) State-Space model.
...
class StateSpace(nn.Module):
    ...
    def __init__(
        ...
        log_dt = torch.rand(self.H) * (math.log(dt_max) - math.log(dt_min)) + math.log(
            dt_min
        )
    ...
    def linear_system_from_krylov(self, u, k):
        ...
        k = k.to(
            u
        )  # if training in half precision, need to go back to float32 for the fft
...
if __name__ == "__main__":
    ...
    lssl = StateSpace(H, N, dt_min=dt_min, dt_max=dt_max).to(device)
    u = torch.randn(L, B, H).to(device)
