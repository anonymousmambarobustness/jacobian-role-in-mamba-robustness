    ...
    def forward(self, x, mask=None, state=None):
        ...
        if type(mask) == torch.Tensor:
            (
                ...
                from_mask,
                to_mask,
            ) = self.create_masks_for_block_sparse_attn(mask, self.block_size)
        else:
    ...
    def bigbird_block_sparse_attention(
        ...
        second_seq_pad = torch.cat(
            [
                ...
                first_context_layer.new_ones(
                    [bsz, 1, 1, n_rand_blocks * to_block_size]
                ),
            ],
            dim=3,
        )
