...
class StackedRNN(SequenceModule):
        ...
        for i in range(n_layers):
            ...
            if skip_connections:
                self.lin_layers.append(
                    self.weight_norm(torch.nn.Linear(d_hidden, d_hidden))
                )
            else:
    ...
    def default_state(self, *batch_shape, device=None):
        return [
            rnn.default_state(*batch_shape, device=device) for rnn in self.rnn_layers
        ]
    ...
    def forward(self, inputs, *args, state=None, **kwargs):
        ...
        next_states = []
        out = 0.0
        for rnn, prev_state, lin, dropout in zip(
            self.rnn_layers, prev_states, self.lin_layers, self.dropout_layers
        ):
            # Run RNN on inputs
...
class LearnedUpsampling1d(torch.nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, bias=True):
        ...
        if bias:
            self.bias = torch.nn.Parameter(torch.FloatTensor(out_channels, kernel_size))
        else:
    ...
    def forward(self, input):
        ...
        bias = (
            ...
            .unsqueeze(2)
            .expand(batch_size, self.conv_t.out_channels, length, kernel_size)
            .contiguous()
            .view(batch_size, self.conv_t.out_channels, length * kernel_size)
        )
...
class SampleRNN(SequenceModule):
    ...
    def default_state(self, batch_size, device=None):
        return [
            rnn.default_state(batch_size, device=device)
            for rnn in self.frame_level_rnns
        ]
    ...
    def step(self, x, state=None, *args, **kwargs):
        ...
        for (i, rnn), state_ in zip(
            reversed(list(enumerate(self.frame_level_rnns))), reversed(state)
        ):
            ...
            if self.reproduce:
                ...
                prev_samples = prev_samples.view(
                    batch_size, -1, rnn.n_frame_samples, self.d_model
                )
            ...
            if i == len(self.frame_level_rnns) - 1:
                ...
                frame_index = (
                    self._step_idx // rnn.n_frame_samples
                ) % self.frame_level_rnns[i + 1].frame_size
                upper_tier_conditioning = self._frame_level_outputs[i + 1][
                    :, frame_index, :
                ].unsqueeze(1)

            upper_tier_conditioning, new_state = rnn(
                prev_samples, upper_tier_conditioning, state_
            )
        ...
        upper_tier_conditioning = self._frame_level_outputs[0][
            :, self._step_idx % bottom_frame_size, :
        ].unsqueeze(1)
    ...
    def forward(self, inputs, *args, state=None, **kwargs):
        ...
        # upper_tier_conditioning shape: (B, M_{last_rnn}, D_HIDDEN) [last rnn]
        return (
            self.sample_level_mlp(mlp_input_sequences, upper_tier_conditioning),
            new_states,
        )
...
class SampleLevelMLP(torch.nn.Module):
    def __init__(
        ...
        if self.reproduce:
            assert (
                embedding
            ), "Original SampleRNN SampleLevelMLP uses an embedding layer."
    ...
    def forward(self, prev_samples, upper_tier_conditioning):
        if self.embedding:
            ...
            prev_samples = self.embedding(prev_samples.contiguous().view(-1)).view(
                prev_samples.shape[0], -1, self.d_model
            )

        assert (
            prev_samples.shape[-1] == self.d_model
        ), "`prev_samples` shape should be (B, L', D_MODEL)"
